...
- What i try to pay attention to most is style. Is the style consistent, variable/class names. Do the classes contain similar methods or do some class specific function lie outside the class, but others are contained in class. I like to check to make the style conforms with the standard style. I check for unused code and commented code lines. Are functions easy to read and if not do they have documentation.
- Containers could make sure everyone starts out with same enviroment. Using containers, like docker, would ensure everyone is using the same packages & specified versions & same os. If you decide that everything works best with everyone coding in the same evironment, then os level testing could be done simply by making a new container with a different os pulling your projects and seeing if it can run. containers can solve a lot of problems involving reporducibility. An important factor though is that docker requires specific hardware so everyone might still need to use similar computers or things could go awry.
